...
9 行目 8 マスすべて：前の行に続き、真下から左へ回り、左下・左・左上を通って真上の手前まで`,
  ...
  'settings.activity.emptyBody': 'CLI またはアプリのセッションを開始すると、完了したモデルの応答がここに表示されます。',
  ...
  'settings.plugins.description': 'インストール済みプラグインを確認し、状態を把握し、現在のランタイムに変更を適用します。',
  'settings.plugins.browserTitle': 'インストール済みプラグインを閲覧',
  'settings.plugins.browserEyebrow': 'プラグインマネージャー',
  'settings.plugins.browserDescription': 'プラグインはスキル、エージェント、フック、MCP サーバー、言語ツールをまとめたものです。ここでインストール済みプラグイン、状態、ランタイムへの適用結果を管理できます。',
  ...
  'scheduledPage.desktopNotice': 'スケジュールタスクは cc-haha サーバーの実行中に動作します。Desktop アプリまたは Web サーバーを起動したままにしてください。',
  ...
  'trace.list.emptyBody': '収集を有効にすると、新しいセッションはモデル呼び出しトレースをサーバーの traces ディレクトリに書き込みます。',
